...
const apiEmpleados: string = apiUrl + "/empleado";

// Referencias de los elementos en el DOM
const empleadosTable: HTMLTableSectionElement = document.getElementById('empleados-table')?.getElementsByTagName('tbody')[0] as HTMLTableSectionElement;
const empleadoForm: HTMLFormElement | null = document.getElementById('empleado-form') as HTMLFormElement;
const empleadoIdInput: HTMLInputElement | null = document.getElementById('empleado-id') as HTMLInputElement;
const empleadoNombreInput: HTMLInputElement | null = document.getElementById('empleado-nombre') as HTMLInputElement;
const empleadoPasswordInput: HTMLInputElement | null = document.getElementById('empleado-password') as HTMLInputElement;

document.getElementById('generar-informes-button')?.addEventListener('click', () => generarModalFechas());

interface Horarios {L?: Horario, M?: Horario, X?: Horario, J?: Horario, V?: Horario, S?: Horario, D?: Horario};
...
const cargarTablaEmpleados = (empleados: Empleado[]): void => {

    empleadosTable.innerHTML = ''; // Limpiar tabla antes de renderizar

    empleados.forEach(empleado => {
      const row: HTMLTableRowElement = empleadosTable.insertRow();
      const dni: string = empleado.dni ? empleado.dni : 'No disponible';
      const nombre: string = empleado.nombre ? empleado.nombre : 'No disponible';
      row.innerHTML = `
        <td>${dni}</td>
        <td>${nombre}</td>
        <td>
...
const loadEmpleados = async (): Promise<void> => {
  
  try {
    
    const jwt: string | null = sessionStorage.getItem('jsonWebToken');
    
    const response: Response = await fetch(apiEmpleados, {
      headers: {
...
const deleteEmpleado = async (id: number): Promise<void> => {
    ...
    try {
      const jwt: string | null = sessionStorage.getItem('jsonWebToken');
      const response: Response = await fetch(`${apiEmpleados}/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${jwt}`
        }
...
const editarEmpleado = async (empleado: Empleado): Promise<void> => {
  ...
  for(let i = 0; i < diasSemana.length; i++) {
    ...
    diaInput.type = 'time';
    const diaKey = abrevs[i] as keyof Horarios;
    diaInput.value = empleado.horarios && empleado.horarios[diaKey]?.entradaDia ? empleado.horarios[diaKey]?.entradaDia : '';
    ...
    nocheInput.value = empleado.horarios && empleado.horarios[diaKey]?.entradaNoche ? empleado.horarios[diaKey]?.entradaNoche : '';
  ...
  confirmButton.addEventListener('click', async () => {
    ...
    for(let i = 0; i < diasSemana.length; i++) {
      ...
      let entradaDia = (document.getElementById(`entrada_dia_${abrevs[i]}`) as HTMLInputElement).value;
      let entradaNoche = (document.getElementById(`entrada_noche_${abrevs[i]}`) as HTMLInputElement).value;
...
const empleadoEditado = async (empleado: Empleado): Promise<void> => {

  const response: Response = await fetch(apiUrl + "/empleado/" + empleado.id, {
    method: "PUT",
    headers: {
...
const generateReport = async (fechaInicio: string, fechaFin: string, id?: number): Promise<void> => {
  try {
    const jwt: string | null = sessionStorage.getItem('jsonWebToken');
    const endpoint: string = id ? `${apiEmpleados}/generarInforme/${id}` : `${apiEmpleados}/generarInforme`;
    const endpointQuery: string = `${endpoint}?fechaInicio=${encodeURIComponent(fechaInicio)}&fechaFin=${encodeURIComponent(fechaFin)}`;
    const response: Response = await fetch(endpointQuery, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${jwt}`
      }
...
const generarModalFechas = async (id?: number) => {
